...
For field-level schema details see [../schema-reference.md](../schema-reference.md).
...
6. **Transforms** — Config artifacts are rendered for each PE that has at
   least one L3VPN site.
...
7. **Merge** — The operator reviews the diff in the Infrahub UI and merges.
   ...
   task (e.g. `invoke lab.push-arista`).
...
## Decommissioning an L3VPN

Decommissioning is not automated in v1. The operator must:

1. Set `ServiceL3Vpn.status` to `decommissioned` and each `ServiceL3VpnSite.status`
   to `decommissioned` on a feature branch.
2. Manually free the `pe_interface` (set status back to `free`).
3. Delete or retain the `IpamVRF`, IP addresses, and BGP sessions as policy
   requires.
4. Open a proposed change and merge.

A future `DecommissionL3VpnGenerator` could automate these steps.
