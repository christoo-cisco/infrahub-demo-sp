...
- `MplsIsisProcess` — `area_id` (Text, default `49.0001`), `level` (Dropdown: `level-1`, `level-2`, `level-1-2`; default `level-2`), `net_id` (Text, stored — pre-computed from the device's loopback IPv4 by the bootstrap data, format `49.0001.<padded-loopback>.00`). Relationship `interfaces → InterfacePhysical` (many) for ISIS-enabled interfaces.
...
- `ServiceL3Vpn`:
  ...
  - `tenant → OrganizationGeneric` (cardinality one, required)
  - `vpn_id` (Number, unique, required) — allocated from `vpn_id_pool` (`CoreNumberPool`) by the catalog before object creation
  - `address_family` (Dropdown: `ipv4`, `ipv4_ipv6`; default `ipv4`)
  - `vrf → IpamVRF` (cardinality one, **optional** — populated by the generator on first run; holds the actual `vrf_rd`, `import_rt`, `export_rt`)
  - `status` (Dropdown: `draft`, `active`, `decommissioned`; default `draft`)
  - `sites → ServiceL3VpnSite` (children, cardinality many)

Fields the user provides at create time: `name`, `description`, `tenant`, `address_family`. The catalog also sets `vpn_id` from the pool. The generator sets `vrf` and may update `status` to `active`.

- `ServiceL3VpnSite` (plain node — does **not** inherit `RoutingProtocol`, to avoid a duplicate `device` relationship vs. the explicit `pe`):
  - `name` (Text, required)
  - `l3vpn` (parent, cardinality one, required)
  - `pe → DcimDevice` (cardinality one, required — user picks at create time)
  - `pe_interface → InterfacePhysical` (cardinality one, **optional** — allocated by the generator)
  - `customer_subnet → IpamPrefix` (cardinality one, required — user provides the CIDR at create time)
  - `pe_address → IpamIPAddress` (cardinality one, **optional** — allocated by the generator from `pe_ce_pool`)
  - `ce_address → IpamIPAddress` (cardinality one, **optional** — allocated by the generator from `pe_ce_pool`)
  - `routing_protocol` (Dropdown: `ebgp`, `static`, `connected`; required)
  - `bgp_peer_asn` (Number, optional — required when `routing_protocol == ebgp`, enforced by check)
  - `static_routes` (`kind: JSON`, optional — list of `{prefix, next_hop}` objects; required when `routing_protocol == static`, enforced by check)
  - `status` (Dropdown: `provisioning`, `active`, `decommissioned`; default `provisioning`)

Fields the user provides at create time: `name`, `l3vpn`, `pe`, `customer_subnet`, `routing_protocol`, conditionally `bgp_peer_asn` or `static_routes`. The generator sets `pe_interface`, `pe_address`, `ce_address`, and may update `status` to `active`.
...
objects/
  ...
  60_backbone.yml            4 PE Devices (pe-lon-arista, pe-fra-cisco, pe-ams-juniper,
                             ...
                             Hand-authored once; documented as the only file that needs editing
                             to add a 5th PE later.
