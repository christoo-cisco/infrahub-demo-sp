...
class CoreCiscoIosXr(InfrahubTransform):
    """Render Cisco IOS-XR core configuration."""
    ...
    async def transform(self, data: dict[str, Any]) -> str:
        ...
        env = Environment(
            ...
            autoescape=select_autoescape(
                disabled_extensions=("j2",), default_for_string=False
            ),
            keep_trailing_newline=True,
            trim_blocks=True,
            lstrip_blocks=True,
        )
        template = env.get_template("core_cisco_iosxr.j2")
